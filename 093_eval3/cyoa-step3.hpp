#ifndef __STORY_STEP3__
#define __STORY_STEP3__
#include <assert.h>
...
class story {
 private:
  ...
  void findwayshelper(size_t current,
                      vector<bool> visited,
                      vector<size_t> path,
                      bool & winnable) {
    ...
    if (pages[current].gettype() == 'W') {
      ...
      for (size_t i = 0; i < pages[current].getopnum(); i++) {
        ...
        int nextpage = existedpage(pages[current].selectoption(i));
        if (!visited[nextpage]) {
          findwayshelper(nextpage, visited, path, winnable);
        }
      }
    }
    path.pop_back();
 ...
 public:
  ...
  void findways() {
    ...
    vector<size_t> path;
    bool winnable = false;
    findwayshelper(startpage, visited, path, winnable);  //recursively
    if (winnable == false) {
